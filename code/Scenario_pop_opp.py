...
def model(opparams, init_vals, period, params, t):
    Sa_0, Sb_0, Sc_0, Sd_0, Ba_0, Bb_0, Vc_0, Vd_0, Eax_0, Eay_0, Ebx_0, Eby_0, Ecx_0, Ecy_0, Edx_0, Edy_0, Iax_0, Iay_0, Ibx_0, Iby_0, Icx_0, Icy_0, Idx_0, Idy_0, Hax_0, Hay_0, Hbx_0, Hby_0, Hcx_0, Hcy_0, Hdx_0, Hdy_0, Uax_0, Uay_0, Ubx_0, Uby_0, Ucx_0, Ucy_0, Udx_0, Udy_0, Ra_0, Rb_0, Rc_0, Rd_0, D_0, Ns_0, Nh_0, Nu_0 = init_vals
    Sa, Sb, Sc, Sd, Ba, Bb, Vc, Vd, Eax, Eay, Ebx, Eby, Ecx, Ecy, Edx, Edy, Iax, Iay, Ibx, Iby, Icx, Icy, Idx, Idy, Hax, Hay, Hbx, Hby, Hcx, Hcy, Hdx, Hdy, Uax, Uay, Ubx, Uby, Ucx, Ucy, Udx, Udy, Ra, Rb, Rc, Rd, D, Ns, Nh, Nu = [Sa_0], [Sb_0], [Sc_0], [Sd_0], [Ba_0], [Bb_0], [Vc_0], [Vd_0], [Eax_0], [Eay_0], [Ebx_0], [Eby_0], [Ecx_0], [Ecy_0], [Edx_0], [Edy_0], [Iax_0], [Iay_0], [Ibx_0], [Iby_0], [Icx_0], [Icy_0], [Idx_0], [Idy_0], [Hax_0], [Hay_0], [Hbx_0], [Hby_0], [Hcx_0], [Hdx_0], [Hdy_0], [Hcy_0], [Uax_0], [Uay_0], [Ubx_0], [Uby_0], [Ucx_0], [Ucy_0], [Udx_0], [Udy_0], [Ra_0], [Rb_0], [Rc_0], [Rd_0], [D_0], [Ns_0], [Nh_0], [Nu_0]
    

    P1, P3, P4, P5, P9 = period
    c, epsilona, epsilonb, epsilonc, epsilond = params
    
    beta  = opparams[:6]
    alpha = opparams[6:12]
    delta = opparams[12:18]
    theta = opparams[18:]
    ...
    for i in t[1:]:
        ...
        next_Ra  = Ra[-1] + ((1-alphaax)*P3*Iax[-1] + (1-alphaa)*P3*Iay[-1] + (1-deltaax)*P4*Hax[-1] + (1-deltaa)*P4*Hay[-1] + (1-thetaax)*P5*Uax[-1] + (1-thetaa)*P5*Uay[-1])*dt
        ...
        next_Rb  = Rb[-1] + ((1-alphabx)*P3*Ibx[-1] + (1-alphab)*P3*Iby[-1] + (1-deltabx)*P4*Hbx[-1] + (1-deltab)*P4*Hby[-1] + (1-thetabx)*P5*Ubx[-1] + (1-thetab)*P5*Uby[-1])*dt
        ...
        next_Rc  = Rc[-1] + ((1-alphacx)*P3*Icx[-1] + (1-alphac)*P3*Icy[-1] + (1-deltacx)*P4*Hcx[-1] + (1-deltac)*P4*Hcy[-1] + (1-thetacx)*P5*Ucx[-1] + (1-thetac)*P5*Ucy[-1])*dt
        ...
        next_Rd  = Rd[-1] + ((1-alphadx)*P3*Idx[-1] + (1-alphad)*P3*Idy[-1] + (1-deltadx)*P4*Hdx[-1] + (1-deltad)*P4*Hdy[-1] + (1-thetadx)*P5*Udx[-1] + (1-thetad)*P5*Udy[-1])*dt
        ...
        Ra  = np.vstack((Ra, next_Ra))
        Rb  = np.vstack((Rb, next_Rb))
        Rc  = np.vstack((Rc, next_Rc))
        Rd  = np.vstack((Rd, next_Rd))
    ...
    return Sa, Sb, Sc, Sd, Ba, Bb, Vc, Vd, Eax, Eay, Ebx, Eby, Ecx, Ecy, Edx, Edy, Iax, Iay, Ibx, Iby, Icx, Icy, Idx, Idy, Hax, Hay, Hbx, Hby, Hcx, Hcy, Hdx, Hdy, Uax, Uay, Ubx, Uby, Ucx, Ucy, Udx, Udy, Ra, Rb, Rc, Rd, D, Ns, Nh, Nu


def cost(opparams, modelparams, idata, cddata, hdata, udata, i):
    init_vals, period, params, N, t = modelparams
    Sa, Sb, Sc, Sd, Ba, Bb, Vc, Vd, Eax, Eay, Ebx, Eby, Ecx, Ecy, Edx, Edy, Iax, Iay, Ibx, Iby, Icx, Icy, Idx, Idy, Hax, Hay, Hbx, Hby, Hcx, Hcy, Hdx, Hdy, Uax, Uay, Ubx, Uby, Ucx, Ucy, Udx, Udy, Ra, Rb, Rc, Rd, D, Ns, Nh, Nu = model(opparams, init_vals, period, params, t)
    dum = 0
...
idata  = df5.values[118:149, 1:7]
cddata = df6.values[118:149, 1:7]
ddata  = df7.values[118:149, 1:7]
hdata  = df8.values[118:149, 1:7]
udata  = df9.values[118:149, 1:7]


file = pd.ExcelFile('BoostS.xlsx')
df1 = file.parse(0) #init
df2 = file.parse(1) #params
df3 = file.parse(2) #Coef

# file = pd.ExcelFile('BoostS.xlsx') # IGRA
# df1 = file.parse(6) #init
# df2 = file.parse(7) #params
# df3 = file.parse(8) #Coef
...
epsilon  = np.array([0., 0.005, 0.01, 0.001, 0.005, 0.005])#np.array([0., 0.01, 0.01, 0.01, 0.01, 0.01])
epsilonv = np.array([0., 0., 0., 0.01, 0.01, 0.01])#np.array([0., 0., 0., 0.001, 0.005, 0.005])

period  = P1, P3, P4, P5, P9
params  = c, epsilona, epsilonb, epsilonc, epsilond
...
init_vals = Sa_0, Sb_0, Sc_0, Sd_0, Ba_0, Bb_0, Vc_0, Vd_0, Eax_0, Eay_0, Ebx_0, Eby_0, Ecx_0, Ecy_0, Edx_0, Edy_0, Iax_0, Iay_0, Ibx_0, Iby_0, Icx_0, Icy_0, Idx_0, Idy_0, Hax_0, Hay_0, Hbx_0, Hby_0, Hcx_0, Hcy_0, Hdx_0, Hdy_0, Uax_0, Uay_0, Ubx_0, Uby_0, Ucx_0, Ucy_0, Udx_0, Udy_0, Ra_0, Rb_0, Rc_0, Rd_0, D_0, Ns_0, Nh_0, Nu_0
fSa, fSb, fSc, fSd, fBa, fBb, fVc, fVd, fEax, fEay, fEbx, fEby, fEcx, fEcy, fEdx, fEdy, fIax, fIay, fIbx, fIby, fIcx, fIcy, fIdx, fIdy, fHax, fHay, fHbx, fHby, fHcx, fHcy, fHdx, fHdy, fUax, fUay, fUbx, fUby, fUcx, fUcy, fUdx, fUdy, fRa, fRb, fRc, fRd, fD, fNs, fNh, fNu  = init_vals
ZH = (alphaax*fIax + alphaa*fIay + alphabx*fIbx + alphab*fIby + alphacx*fIcx + alphac*fIcy + alphadx*fIdx + alphad*fIdy) * P3
   

opparams = np.ones(24)*0.001
opp = np.ones((32, 24))*0.001
for i in range(31):
    if i > 0:
        ...
        nSa, nSb, nSc, nSd, nBa, nBb, nVc, nVd, nEax, nEay, nEbx, nEby, nEcx, nEcy, nEdx, nEdy, nIax, nIay, nIbx, nIby, nIcx, nIcy, nIdx, nIdy, nHax, nHay, nHbx, nHby, nHcx, nHcy, nHdx, nHdy, nUax, nUay, nUbx, nUby, nUcx, nUcy, nUdx, nUdy, nRa, nRb, nRc, nRd, nD, nNs, nNh, nNu = model(opp[i], init_vals, period, params, dummyt)
        init_vals =  nSa[-1], nSb[-1], nSc[-1], nSd[-1], nBa[-1], nBb[-1], nVc[-1], nVd[-1], nEax[-1], nEay[-1], nEbx[-1], nEby[-1], nEcx[-1], nEcy[-1], nEdx[-1], nEdy[-1], nIax[-1], nIay[-1], nIbx[-1], nIby[-1], nIcx[-1], nIcy[-1], nIdx[-1], nIdy[-1], nHax[-1], nHay[-1], nHbx[-1], nHby[-1], nHcx[-1], nHcy[-1], nHdx[-1], nHdy[-1], nUax[-1], nUay[-1], nUbx[-1], nUby[-1], nUcx[-1], nUcy[-1], nUdx[-1], nUdy[-1], nRa[-1], nRb[-1], nRc[-1], nRd[-1], nD[-1], nNs[-1], nNh[-1], nNu[-1] 
        
        
        epsilon  = np.array([0., 0.005, 0.01, 0.001, 0.005, 0.005])
        epsilonv = np.array([0., 0., 0., 0.01, 0.01, 0.01])
        ...
        fRa  = np.vstack((fRa, nRa[1:,:]))
        fRb  = np.vstack((fRb, nRb[1:,:]))
        fRc  = np.vstack((fRc, nRc[1:,:]))
        fRd  = np.vstack((fRd, nRd[1:,:]))
